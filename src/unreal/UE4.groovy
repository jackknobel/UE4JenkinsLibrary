...
{
	bat "${EngineUAT} BuildCookRun -project=${ProjectFile} -platform=${platform} -skipcook -skipbuild -nocompileeditor -NoSubmit -stage -package -clientconfig=" + buildConfiguration.name() + additionalArguments + " -StagingDirectory=\"${outputDir}\"" + (usePak ? " -pak " : " ") +  (iterative ? " -iterativedeploy" : "")
}
...
{
	Deploy("XboxOne", buildConfiguration, true, outputDir, usePak, iterative, "-cmdline=-Messaging -device=XboxOne@" + consoleIP + " " + additionalArguments)
}
